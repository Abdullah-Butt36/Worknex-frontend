...
const Navbar = () => {
  ...
  return (
    <nav className="w-full h-[72px] bg-[#0B1221] border-b border-gray-800 flex items-center justify-between px-4 md:px-10 lg:px-16 sticky top-0 z-[1000] font-sans transition-all">
      ...
      <div
        className={`flex items-center gap-2 md:gap-6 ${isSearchOpen ? "w-full" : ""}`}
      >
        {/* Search Logic */}
        <div
          className={`${isSearchOpen ? "flex w-full" : "hidden md:flex"} items-center bg-[#1F2937]/40 border border-gray-700 rounded-full px-4 py-1.5 lg:w-[240px]`}
        >
          ...
          {isSearchOpen && (
            <X
              size={16}
              className="text-gray-400 ml-2 cursor-pointer md:hidden"
