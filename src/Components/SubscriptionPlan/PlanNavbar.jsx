...
const PlanNavbar = () => {
  ...
  const navLinks = [
    { name: 'Home', to: '/' },
    { name: 'Explore', to: '/opportunities-feed' },
    { name: 'Enterprise', to: '/wizard-1' }
  ];
      ...
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link 
              key={link.name} 
              to={link.to} 
              className="text-slate-400 font-semi-bold hover:text-white text-[17px] transition-colors"
            >
              {link.name}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3 md:gap-6">
          <button 
            ...
            className="text-slate-400 text-[13px] md:text-[14px] font-semibold hover:text-white transition-colors  tracking-wider"
          >
            Log In
          </button>
          <button 
            onClick={() => navigate('/role-selection')}
            className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 md:px-7 md:py-2.5 rounded-full text-[13px] md:text-[13px] font-black   transition-all shadow-[0_0_20px_rgba(37,99,235,0.2)] active:scale-95"
          >
            Sign Up
          </button>
